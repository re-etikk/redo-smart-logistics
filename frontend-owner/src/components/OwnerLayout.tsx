...
import {
  ...
  FileText, Star, Headset, Settings, Bell, Wallet, ChevronDown, User, Gift, ShieldCheck, Plus, Landmark, Box
} from "lucide-react";
        ...
        <div className="mx-auto max-w-[1500px] px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden xl:flex items-center gap-5 text-xs font-bold text-slate-700">
              <Link to="/dashboard" className={`transition hover:text-amber-600 ${location.pathname === "/dashboard" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Dashboard
              </Link>
              <Link to="/trucks" className={`transition hover:text-amber-600 ${location.pathname === "/trucks" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                My Trucks
              </Link>
              <Link to="/loads" className={`transition hover:text-amber-600 ${location.pathname === "/loads" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Find Loads
              </Link>
              <Link to="/bookings" className={`transition hover:text-amber-600 ${location.pathname === "/bookings" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                My Bookings
              </Link>
              <Link to="/earnings" className={`transition hover:text-amber-600 ${location.pathname === "/earnings" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Earnings
              </Link>
              <Link to="/trips" className={`transition hover:text-amber-600 ${location.pathname === "/trips" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Trips
              </Link>
              <Link to="/payments" className={`transition hover:text-amber-600 ${location.pathname === "/payments" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Payments
              </Link>
              <Link to="/support" className={`transition hover:text-amber-600 ${location.pathname === "/support" ? "text-amber-600 border-b-2 border-amber-400 pb-1 font-black" : ""}`}>
                Support
              </Link>
            </nav>
          </div>
            ...
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 py-1 font-bold text-xs space-y-1">
                  ...
                  <Link to="/documents" onClick={() => setProfileOpen(false)} className="px-4 py-2 hover:bg-slate-50 flex items-center gap-2 text-slate-700">
