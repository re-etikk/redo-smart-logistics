...
export default function Login() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Dev fallback — go to dashboard anyway
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white border border-slate-200/80 rounded-3xl p-8 shadow-xl space-y-6">
          <div className="text-center space-y-1">
            ...
            <p className="text-xs text-slate-500">Login to your REDO Customer account</p>
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["google", "facebook", "apple"] as const).map(p => (
                <button
                  ...
                  className="py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 capitalize"
                >
                  {p}
                </button>
              ))}
      ...
      <footer className="py-6 border-t border-slate-200 bg-white">
        <div className="max-w-4xl mx-auto px-4 grid grid-cols-3 gap-4 text-center text-xs text-slate-600">
          <div className="flex items-center justify-center gap-2">
            ...
            <span><strong>Verified Trucks</strong></span>
          </div>
          <div className="flex items-center justify-center gap-2">
