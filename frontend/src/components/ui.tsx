...
type Tone = "neutral" | "ok" | "warn" | "danger" | "accent" | "info" | "purple";

export function Button({ variant = "primary", className = "", children, ...props }:
  ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary" | "secondary" | "ghost" | "danger" }) {
  const styles = {
    primary: "bg-accent text-accent-fg hover:brightness-95 active:scale-[.99]",
...
export function Badge({ tone = "neutral", children }: { tone?: Tone; children: ReactNode }) {
  const tones: Record<Tone, string> = {
    ...
    info: "bg-info-soft text-info", purple: "bg-purple-soft text-purple",
  };
...
export function ReasonChips({ reasons }: { reasons: string[] }) {
  return (
    <div className="flex flex-wrap gap-1.5">
      {reasons.map((r) => (
        <span key={r} className="rounded-md bg-accent-soft text-ink px-2 py-0.5 text-[11px] font-semibold">{r}</span>
      ))}
    </div>
  );
}
...
export function StatCard({ icon: Icon, label, value, sub, tone = "accent" }: {
  icon: LucideIcon; label: string; value: string | number; sub?: ReactNode;
  tone?: "accent" | "ok" | "info" | "warn" | "danger" | "purple";
}) {
  const tile: Record<string, string> = {
    accent: "bg-accent-soft text-accent", ok: "bg-ok-soft text-ok", info: "bg-info-soft text-info",
    warn: "bg-warn-soft text-warn", danger: "bg-danger-soft text-danger", purple: "bg-purple-soft text-purple",
  };
  return (
    <Card className="p-4 flex items-center gap-3">
      <span className={`h-11 w-11 rounded-xl grid place-items-center ${tile[tone]}`}><Icon size={20} /></span>
      <div className="min-w-0">
        <p className="text-xs font-semibold text-ink-faint">{label}</p>
        <p className="text-xl font-extrabold text-ink leading-tight tabular-nums">{value}</p>
        {sub && <p className="text-[11px] font-medium text-ink-faint">{sub}</p>}
      </div>
    </Card>
  );
}
    ...
    <div className="flex gap-1 border-b border-line overflow-x-auto" role="tablist">
      {tabs.map((t) => (
        <button key={t.key} role="tab" aria-selected={active === t.key} onClick={() => onChange(t.key)}
          className={`whitespace-nowrap px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition
            ${active === t.key ? "border-accent text-ink" : "border-transparent text-ink-faint hover:text-ink"}`}>
          {t.label}{t.count !== undefined && <span className="ml-1 text-ink-faint">({t.count})</span>}
        </button>
      ))}
    </div>
  );
...
export function SearchInput({ value, onChange, placeholder }: {
  ...
  return (
    <div className="relative flex-1 min-w-[220px]">
      <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-faint" width="16" height="16"
        viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
        <circle cx="11" cy="11" r="7" /><path d="m21 21-4-4" strokeLinecap="round" />
      </svg>
      <input className={inputCls + " pl-9"} value={value} placeholder={placeholder}
