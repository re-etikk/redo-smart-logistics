...
export default function ProfileSettings() {
  ...
  // Shipper Settings View
  return (
    <Layout>
      <div className="max-w-2xl mx-auto py-8">
        <h1 className="text-2xl font-black text-slate-900">Profile Settings</h1>
        <p className="text-xs text-slate-500 mt-1">Manage your customer account and company preferences.</p>
        <div className="mt-6 bg-white border border-slate-200 rounded-2xl p-6 space-y-4 shadow-sm">
          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1">Company / Full Name</label>
            <input defaultValue={profile?.full_name || "REDO Customer"} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold" />
          </div>
          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1">Phone Number</label>
            <input defaultValue={profile?.phone || "+91 98765 43210"} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold" />
          </div>
