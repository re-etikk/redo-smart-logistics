...
export const APP_ROLE = 'truck_owner'; // customer app uses 'sme'
...
export function LoginScreen({ onDone, goSignup }: { onDone: () => void; goSignup: () => void }) {
  ...
  const demoLogin = async () => {
    setBusy(true);
    try {
      let { error } = await supabase.auth.signInWithPassword({ email: 'driver@redo.app', password: 'Password@123' });
      if (error) {
        await supabase.auth.signUp({
          email: 'driver@redo.app',
          password: 'Password@123',
          options: { data: { full_name: 'Harpreet Singh (Demo)' } },
        });
        const res = await supabase.auth.signInWithPassword({ email: 'driver@redo.app', password: 'Password@123' });
        error = res.error;
      }
      if (error) {
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: C.canvas }}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 20 }}>
        ...
        <Card>
          ...
          <View style={{ height: 10 }} />
          <Button title="⚡ 1-Tap Quick Demo Driver Login" variant="secondary" onPress={demoLogin} />
          <View style={{ height: 10 }} />
