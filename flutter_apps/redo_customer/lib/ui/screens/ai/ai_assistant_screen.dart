...
class AiAssistantScreen extends StatefulWidget {
  ...
  const AiAssistantScreen({super.key, this.onTabChangeRequested});
  ...
  void initState() {
    ...
    Future.microtask(() {
      if (mounted) {
        ...
        voice.setActiveShipmentsProvider(() => context.read<ShipmentsViewModel>().shipments);
      }
    });
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text('⚡ Book This Corridor', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w900)),
              onPressed: () {
                final bvm = context.read<BookingViewModel>();
                bvm.setOrigin(from);
                bvm.setDestination(to);
                if (action.weightTons != null) {
                  bvm.setWeightTons(action.weightTons!);
                }
                if (widget.onTabChangeRequested != null) {
                  widget.onTabChangeRequested!(0);
                } else {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const MatchingTrucksScreen()));
                }
              },
            ),
          ),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                hasBooking ? '📍 View Live GPS on Map' : '+ Book a Truck Now',
                style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w800),
              ),
              onPressed: () {
                if (hasBooking) {
                  final shipments = context.read<ShipmentsViewModel>().shipments;
                  final match = shipments.firstWhere(
                    (s) => s.id == id,
                    orElse: () => shipments.isNotEmpty
                        ? shipments.first
                        : BookingItem(
                            id: id,
                            cargoId: id,
                            ...
                            weightTons: 10,
                            agreedPriceInr: 25000,
                            status: status,
                            createdAt: DateTime.now().toIso8601String(),
                          ),
                  );
                  Navigator.push(context, MaterialPageRoute(builder: (_) => TrackingScreen(booking: match)));
                } else {
                  if (widget.onTabChangeRequested != null) {
                    widget.onTabChangeRequested!(0);
                  } else {
