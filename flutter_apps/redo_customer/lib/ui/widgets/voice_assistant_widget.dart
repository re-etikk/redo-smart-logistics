...
/// A floating voice assistant button that overlays all screens in REDO Customer app.
...
/// Dedicated Text-to-Text AI Assistant Bottom Sheet for Customer App
...
class _AiChatBottomSheetState extends State<AiChatBottomSheet> {
  ...
  final _quickPrompts = [
    ...
    'Book Mumbai to Delhi truck 🚛',
    'Check freight market rates 🏷️',
    'Download GST invoice 📄',
    'Driver live location & status ⚡',
  ];

  Widget _buildTrackingCard(ChatMessage m, VoiceAssistantService va, bool isDark, Color textPrimary) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.navigation, size: 10, color: AppColors.success),
                    const SizedBox(width: 4),
                    Text(
                      'In Transit • Live GPS',
                      style: GoogleFonts.inter(
          ...
          Row(
            children: [
              const Icon(Icons.person, size: 13, color: AppColors.inkMuted),
              const SizedBox(width: 4),
              Text(
                'Ramesh Singh (MH 12 RN 4402)',
                style: GoogleFonts.inter(
                  fontSize: 11,
                  color: AppColors.inkMuted,
                  fontWeight: FontWeight.w600,
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  if (va.onActionReady != null) {
                    va.onActionReady!(const VoiceAssistantAction(
                      type: 'track',
                      responseText: 'Opening live tracking',
                    ));
                  }
                },
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (!m.isUser) ...[
                                if (m.action?.type == 'track' ||
                                    m.text.toLowerCase().contains('track') ||
                                    m.text.toLowerCase().contains('shipment') ||
                                    m.text.toLowerCase().contains('driver') ||
                                    m.text.toLowerCase().contains('mumbai') ||
                                    m.text.toLowerCase().contains('delhi'))
                                  _buildTrackingCard(m, va, isDark, textPrimary),
                                const SizedBox(height: 6),
                                Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
...
/// Reveals [text] a few characters at a time, ChatGPT-style.
