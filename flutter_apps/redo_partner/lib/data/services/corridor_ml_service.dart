...
/// Classification of how a customer load relates to the driver's planned corridor.
enum EnRouteType {
  ...
  enRoutePickup,   // Intermediate pickup, drops at driver's destination (e.g. Lucknow -> Delhi on Patna -> Delhi)
...
/// Result of ML corridor evaluation for an available load against a driver's route.
...
class CorridorMLService {
  ...
  // Persistent dynamic weights learned through driver interactions
  final Map<String, double> _corridorAffinityWeights = {};
  double _learnedDetourToleranceKm = 35.0; // default 35 km max acceptable detour
  ...
  /// Predefined Indian National Highway Corridor Topologies (Ordered Waypoint Sequences)
  /// Each corridor defines a sequence of hubs along major NH / Expressways.
  static final List<List<String>> _knownCorridors = [
    // NH-19 / Purvanchal / Agra-Lucknow: Eastern Freight Corridor
    ...
    // Direct Purvanchal - Lucknow - Agra - Delhi highway corridor
    ['patna', 'buxar', 'varanasi', 'ayodhya', 'gorakhpur', 'lucknow', 'kanpur', 'etawah', 'agra', 'noida', 'delhi'],
    // NH-48 / Western Freight Corridor
    ...
    // NH-16: Eastern Coastal Corridor
    ...
    // NH-52 / NH-47: Central & Southern Industrial Corridors
  ...
  /// Normalizes city string for topological sequence comparison
  ...
  /// Evaluates whether a customer load lies along the driver's planned corridor.
  /// Computes detour, highway waypoint sequence, and returns an ML scored match.
  CorridorMatchResult evaluateMatch({
    required String driverFrom,
    required String driverTo,
    required AvailableLoad load,
    double truckCapacityTons = 16.0,
  }) {
    final dFrom = normCity(driverFrom);
    final dTo = normCity(driverTo);
    final lFrom = normCity(load.origin);
    final lTo = normCity(load.destination);
    ...
    if (_isCityMatch(dFrom, lFrom) && _isCityMatch(dTo, lTo)) {
      ...
      return CorridorMatchResult(
        ...
        description: 'Direct match for your corridor (${load.origin} → ${load.destination})',
    ...
    // 3. Topological Corridor Waypoint Search (Patna -> Lucknow -> Delhi)
    ...
    if (topoMatch != null) {
      ...
      switch (topoMatch.type) {
        case EnRouteType.enRouteDropoff:
          baseScore = 96;
          badge = 'En-Route Dropoff (${load.destination})';
          desc = 'Pickup at origin (${load.origin}) and drop off en-route to $driverTo';
          reasons = [
        ...
        case EnRouteType.enRoutePickup:
          baseScore = 93;
          badge = 'En-Route Pickup (${load.origin})';
          desc = 'Pickup at intermediate hub (${load.origin}) and deliver to $driverTo';
          reasons = [
        ...
        case EnRouteType.enRouteSegment:
          ...
          desc = 'Carry cargo between ${load.origin} and ${load.destination} along your route';
          reasons = [
    ...
    // 4. Geometrical Lat/Lng Detour Calculation (Handles any town/village across India)
    ...
    final pLoadOrigin = RoutingService.getCoordinatesForCity(load.origin);
    final pLoadDest = RoutingService.getCoordinatesForCity(load.destination);
  ...
  /// Verifies if a city matches (contains or identical)
  ...
  /// Checks topological order along known highway corridors.
  _TopologyMatch? _checkTopologySequence({
    ...
    for (final corridor in _knownCorridors) {
      ...
      if (idxDOrig != -1 && idxDDest != -1 && idxDOrig < idxDDest) {
        // Driver is moving forward along this corridor
        ...
        if (idxLOrig != -1 && idxLDest != -1 && idxLOrig < idxLDest) {
          // Load is also moving in forward direction along same corridor!
  ...
  /// Mathematical Geometrical Detour Calculation using Haversine & Vector Cross-Track
  CorridorMatchResult _evaluateGeometricDetour({
    ...
    // Check directionality via dot product along travel vector
    final dotOrigin = _dotProduct(driverOrigin, driverDest, loadOrigin);
    final dotDest = _dotProduct(driverOrigin, driverDest, loadDest);

    // Load origin must be encountered before or at load destination
    final forwardDirection = (dotOrigin <= dotDest + 0.05);

    // If detour is within learned tolerance (or <= 15% of direct distance) and direction is forward
  ...
  /// Calculates composite ML match score (0..99)
  int _calculateScore({
    ...
    // 1. Detour penalty: -1 point per 5 km detour
    final detourPenalty = (detourKm / 5.0).clamp(0.0, 20.0);

    // 2. Capacity fit: optimal when load fills 40% - 95% of truck
    double capBonus = 0;
    if (truckCapacity > 0) {
      ...
      if (ratio >= 0.4 && ratio <= 0.95) {
        ...
        capBonus = -10.0; // overload penalty
      }
    }
  ...
  /// Records driver feedback to dynamically adapt the ML weights over time.
